...
@click.argument('dino-model-string', type=str)
@click.argument('desired-dinov2-size', type=(click.Tuple([int, int])))
def main(episode_idx, path_to_demo_root_dir, demo_name, dino_model_string, desired_dinov2_size):
    # episode_idx = 0
    # path_to_demo_root_dir = Path("/mnt/crucialSSD/datasetsSSD/fish_datasets/simulated/teleop/FISH/expert_demos/frankagym/FrankaInsertion-v1")
    # demo_name = "1232_sim_w_recovery_demos_leftof4thbook_springbookends_graspedrand_noenvrand_noslotrand_20hz_act"
    # dino_model_string = 'dinov2_vitb14_reg'
    ...
    dino_model_string_with_rescale = f"{dino_model_string}_rescale_{rescale_factor}"
    #%%
    dinov2_resize = (((original_image_size[0]*rescale_factor) // patch_size)*patch_size, ((original_image_size[1]*rescale_factor) // patch_size)*patch_size)
    dinov2_output_size = (int(dinov2_resize[0] // patch_size), int(dinov2_resize[1] // patch_size))
    # convert back to int
    dinov2_resize = (int(dinov2_resize[0]), int(dinov2_resize[1]))
    #%%
    # # #################################
    ...
    # transform = T.Compose([
    #     # T.ToPILImage(),
    #     T.Resize(dinov2_resize, interpolation=T.InterpolationMode.BICUBIC),
    ...
    # pca_features = pca.fit_transform(features['x_norm_patchtokens'][0].cpu().numpy())
    # #%%
    ...
    # pca_features_rgb = pca_features_rgb.reshape(*dinov2_output_size, 3)
    # plt.imshow(pca_features_rgb)
    # #%%
    # pca_features_single_channel = pca_features[:, 0].reshape(*dinov2_output_size)
    ...
    # pca_features_single_channel = pca_features_single_channel.reshape(*dinov2_output_size, 1)
    ...
    episode_data_dinov2_group_name = episode_data_group_name + '/' + dino_model_string_with_rescale
    if dino_model_string_with_rescale not in zarr_dataset[episode_data_group_name]:
        zarr_dataset.create_group(episode_data_dinov2_group_name)
        print(f"Created group {dino_model_string_with_rescale} in episode data group {episode_data_group_name} in zarr dataset.")

    x_norm_clstoken_array_name = episode_data_dinov2_group_name + '/observation.x_norm_clstoken'
    # if x_norm_clstoken_array_name not in zarr_dataset:
    zarr_dataset.create_array(x_norm_clstoken_array_name, shape=(0, 1, dinov2_model.num_features), dtype=np.float32, chunks=(1, 1, dinov2_model.num_features), compressors=compressors, overwrite=True)
    print(f"Created dataset {x_norm_clstoken_array_name} in zarr dataset.")
    x_norm_regtokens_array_name = episode_data_dinov2_group_name + '/observation.x_norm_regtokens'
    # if x_norm_regtokens_array_name not in zarr_dataset:
    zarr_dataset.create_array(x_norm_regtokens_array_name, shape=(0, dinov2_model.num_register_tokens, dinov2_model.num_features), dtype='float32', chunks=(1, dinov2_model.num_register_tokens, dinov2_model.num_features), compressors=compressors, overwrite=True)
    print(f"Created dataset {x_norm_regtokens_array_name} in zarr dataset.")
    x_norm_patchtokens_array_name = episode_data_dinov2_group_name + '/observation.x_norm_patchtokens'
    # if x_norm_patchtokens_array_name not in zarr_dataset:
    zarr_dataset.create_array(x_norm_patchtokens_array_name, shape=(0, *dinov2_output_size, dinov2_model.num_features), dtype='float32', chunks=(1, *dinov2_output_size, dinov2_model.num_features), compressors=compressors, overwrite=True)
    ...
    for i, batch in tqdm(enumerate(dataloader)):
        ...
        with torch.inference_mode():
            color_image_for_dino = dino_transform(color_image)
            # rgb_image_tensor = transform(rgb_image).unsqueeze(0).to(device)
            features = dinov2_model.forward_features(color_image_for_dino)
            patch_features = einops.rearrange(features['x_norm_patchtokens'], 'b (h w) c -> b h w c', h=dinov2_output_size[0], w=dinov2_output_size[1])
