...
#%%
dir_for_visualizations = Path("./teaser_video_visualizations")
...
quality = 8
crop_lower_x = 125
crop_upper_x = crop_lower_x + 380
crop_lower_y = 50
crop_upper_y = crop_lower_y + 380
#%%
with imageio.get_writer(str(dir_for_visualizations / "teaser_video.mp4"), fps=fps, quality=quality) as writer:
    # for scratch_image_idx in range(0, len(list_of_rgb_images)):
    for scratch_image_idx in range(start_image_idx, end_image_idx):
...
min_contact_prob = 0.0025
max_contact = 0.06
contact_blend_alpha = 0.5
with imageio.get_writer(str(dir_for_visualizations / "contact_map_video.mp4"), fps=fps, quality=quality) as writer:
    for contact_image_idx in range(start_image_idx, end_image_idx):
        ...
        no_contact_mask = contact_map < min_contact_prob
        # use colormap to visualize contact map
        contact_map = np.clip(contact_map, min_contact_prob, max_contact)
        contact_map = (contact_map - min_contact_prob) / (max_contact - min_contact_prob)
        contact_map = (contact_map * 255).astype(np.uint8)
        contact_map_viz = cv2.applyColorMap(contact_map, cv2.COLORMAP_WINTER)
        ...
        writer.append_data(contact_underlay_rgb)
...
with imageio.get_writer(str(dir_for_visualizations / "obj_mask_video.mp4"), fps=fps, quality=quality) as writer:
    for mask_image_idx in range(start_image_idx, end_image_idx):
        ...
        writer.append_data(mask_underlay_rgb)
...
# compute the pca from the first frame and carry it over to all subsequent frames
first_frame_rgb = cv2.imread(str(list_of_rgb_images[start_image_idx]))
...
color_image_for_theia = theia_transform(first_frame_rgb).to(device)
...
pca = pca.fit(patch_features[0].cpu().numpy())
with imageio.get_writer(str(dir_for_visualizations / "theia_features_video.mp4"), fps=fps, quality=quality) as writer:
    for theia_image_idx in range(start_image_idx, end_image_idx):
        ...
        pca_features = pca.transform(patch_features[0].cpu().numpy())
        ...
        writer.append_data(rescaled_pca_features_rgb)
