...
new_zarr_root = zarr.open(str(new_zarr_path), mode='w', zarr_format=3)
#%%
...
def recursive_copy_and_reshard_arrays(original_group, new_group, shard_factor=None, dataset_compressor=zarr.codecs.BloscCodec(cname='zstd', clevel=3, shuffle=zarr.codecs.BloscShuffle.shuffle)):
    # copy the attributes of the group
    for attr_key in original_group.attrs.keys():
        new_group.attrs[attr_key] = original_group.attrs[attr_key]
    for key in original_group.keys():
        if isinstance(original_group[key], zarr.Group):
            if key not in new_group:
                ...
                recursive_copy_and_reshard_arrays(original_group[key], new_group[key], shard_factor=shard_factor, dataset_compressor=dataset_compressor)
            ...
            if key not in new_group:
                ...
                dtype_is_object = True if dataset_dtype == object else False
                ...
                for i in tqdm(range(original_group[key].shape[0])):
                    if dtype_is_object:
                        new_group[key].append(original_group[key][i:i+1].astype("unicode"))
                    else:
